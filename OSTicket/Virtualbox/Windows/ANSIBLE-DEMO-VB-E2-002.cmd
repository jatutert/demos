...
::
::
:: ANSIBLE Virtualbox Demo Configuration Script
:: Version 0.0.2
:: 
:: Date May 02 2024
:: Author John Tutert
...
::      Version 001
::      May 1 2024
::      - duplicate from Docker Demo config script
::
::      Version 002
::      May 2 2024
::      - Change ports to 3 ports instead of 1
::      - PATH extension virtualbox
::
::
:: 
@echo off
...
set ANSIBLE_DEMO_VM_DIRECTORY="D:\Virtual-Machines\Oracle-VM-Virtualbox\Linux\Ansible-Demo"
...
::	Poort SSH
set ANSIBLE_DEMO_VM_CNTRL_SSH_PORT=3001
set ANSIBLE_DEMO_VM_H10_SSH_PORT=3002
set ANSIBLE_DEMO_VM_H11_SSH_PORT=3003
::
::
...
@vagrant box add generic/ubuntu2204 --box-version %ANSIBLE_DEMO_VAGRANT_HDU_TEMPLATE_VER% --clean --provider virtualbox >nul 2>&1
:: 
...
:: 	Virtuele machine 
@VBoxManage closemedium disk %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
@VBoxManage closemedium disk %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H10_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
@VBoxManage closemedium disk %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H11_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
::
::
...
:: 
@mkdir "%ANSIBLE_DEMO_VM_DIRECTORY%"\ >nul 2>&1
::
...
:: @%VIRTBOX_VBOXMAN%\VBoxManage createvm --name "%DOCKER_DEMO_VM_HOSTNAME%" --basefolder %ANSIBLE_DEMO_VM_DIRECTORY%\ --default --ostype "Ubuntu22_LTS_64" --register >nul 2>&1
::
::
@VBoxManage createvm --name "%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%" --basefolder "%ANSIBLE_DEMO_VM_DIRECTORY%" --default --ostype "Ubuntu22_LTS_64" --register >nul 2>&1
@VBoxManage createvm --name "%ANSIBLE_DEMO_VM_H10_HOSTNAME%" --basefolder "%ANSIBLE_DEMO_VM_DIRECTORY%" --default --ostype "Ubuntu22_LTS_64" --register >nul 2>&1
@VBoxManage createvm --name "%ANSIBLE_DEMO_VM_H11_HOSTNAME%" --basefolder "%ANSIBLE_DEMO_VM_DIRECTORY%" --default --ostype "Ubuntu22_LTS_64" --register >nul 2>&1
::
::
::
::
...
@copy "%ANSIBLE_DEMO_VAGRANT_HDU_TEMPLATE_DIR%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%" "%ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%"
@copy "%ANSIBLE_DEMO_VAGRANT_HDU_TEMPLATE_DIR%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%" "%ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H10_HOSTNAME%"
@copy "%ANSIBLE_DEMO_VAGRANT_HDU_TEMPLATE_DIR%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%" "%ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H11_HOSTNAME%"
::
:: ## Virtual Hardisk voorzien van nieuwe UUID
@VBoxManage internalcommands sethduuid %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
@VBoxManage internalcommands sethduuid %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H10_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
@VBoxManage internalcommands sethduuid %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H11_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE% >nul 2>&1
::
:: ## VM Aanpassen zodat VDI wordt gebruikt 
@VBoxManage storageattach "%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%" --storagectl "SATA" --port 0 --device 0 --type hdd --medium %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_CNTRL_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%
@VBoxManage storageattach "%ANSIBLE_DEMO_VM_H10_HOSTNAME%" --storagectl "SATA" --port 0 --device 0 --type hdd --medium %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H10_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%
@VBoxManage storageattach "%ANSIBLE_DEMO_VM_H11_HOSTNAME%" --storagectl "SATA" --port 0 --device 0 --type hdd --medium %ANSIBLE_DEMO_VM_DIRECTORY%\%ANSIBLE_DEMO_VM_H11_HOSTNAME%\%ANSIBLE_DEMO_VAGRANT_VMDK_FILE%
::
::
...
@VBoxManage -q controlvm %ANSIBLE_DEMO_VM_CNTRL_HOSTNAME% natpf1 guestssh,tcp,,%ANSIBLE_DEMO_VM_CNTRL_SSH_PORT%,,22
@VBoxManage -q controlvm %ANSIBLE_DEMO_VM_H10_HOSTNAME% natpf1 guestssh,tcp,,%ANSIBLE_DEMO_VM_H10_SSH_PORT%,,22
@VBoxManage -q controlvm %ANSIBLE_DEMO_VM_H11_HOSTNAME% natpf1 guestssh,tcp,,%ANSIBLE_DEMO_VM_H11_SSH_PORT%,,22
::
::
...
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% -o StrictHostKeyChecking=no sudo sed 's@"1"@"0"@' -i /etc/apt/apt.conf.d/20auto-upgrades
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% -o StrictHostKeyChecking=no sudo sed 's@"1"@"0"@' -i /etc/apt/apt.conf.d/20auto-upgrades
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% -o StrictHostKeyChecking=no sudo sed 's@"1"@"0"@' -i /etc/apt/apt.conf.d/20auto-upgrades
:: 
::
::	#### Configuratie DNS ivm EduRoam
::
@echo Aanpassen DNS configuratie ivm EduRoam 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% sudo sed "s@4.2.2.1@145.76.2.75@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% sudo sed "s@4.2.2.2@145.76.2.85@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% sudo sed "s@208.67.220.220@145.2.14.10, 8.8.8.8, 8.8.4.4@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% sudo netplan apply
::
@echo Aanpassen DNS configuratie ivm EduRoam 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% sudo sed "s@4.2.2.1@145.76.2.75@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% sudo sed "s@4.2.2.2@145.76.2.85@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% sudo sed "s@208.67.220.220@145.2.14.10, 8.8.8.8, 8.8.4.4@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% sudo netplan apply
::
@echo Aanpassen DNS configuratie ivm EduRoam 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% sudo sed "s@4.2.2.1@145.76.2.75@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% sudo sed "s@4.2.2.2@145.76.2.85@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% sudo sed "s@208.67.220.220@145.2.14.10, 8.8.8.8, 8.8.4.4@" -i /etc/netplan/01-netcfg.yaml
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% sudo netplan apply
::
::
::	#### Configuratie eth1 nic 
::
@echo Netwerkkaart eth1 activeren ... 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% "sudo curl -s -o /etc/netplan/00-installer-config.yaml https://raw.githubusercontent.com/jatutert/demos/main/Docker/Virtualbox/Linux/Netplan/00-installer-config.yaml"
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_CNTRL_SSH_PORT% "sudo netplan apply"
:: 
::
@echo Netwerkkaart eth1 activeren ... 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% "sudo curl -s -o /etc/netplan/00-installer-config.yaml https://raw.githubusercontent.com/jatutert/demos/main/Docker/Virtualbox/Linux/Netplan/00-installer-config.yaml"
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H10_SSH_PORT% "sudo netplan apply"
:: 
::
@echo Netwerkkaart eth1 activeren ... 
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% "sudo curl -s -o /etc/netplan/00-installer-config.yaml https://raw.githubusercontent.com/jatutert/demos/main/Docker/Virtualbox/Linux/Netplan/00-installer-config.yaml"
@ssh -i %USERPROFILE%\.ssh\id_rsa %ANSIBLE_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p %ANSIBLE_DEMO_VM_H11_SSH_PORT% "sudo netplan apply"
:: 
