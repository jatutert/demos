...
@VBoxManage closemedium disk %DOCKER_DEMO_VIRTMACH%\%DOCKER_DEMO_VDI_TEMPLATE_FILE% >nul 2>&1
::
...
::
@VBoxManage createvm --name "ulx-s-2204-d-srvr" --basefolder "%DOCKER_DEMO_VIRTMACH%\Controller" --default --ostype "Ubuntu22_LTS_64" --register 
::
...
copy "%DOCKER_DEMO_HDU_TEMPLATE_DIR%\%DOCKER_DEMO_VDI_TEMPLATE_FILE%" "%DOCKER_DEMO_VIRTMACH%"
::
:: Virtual Hardisk voorzien van nieuwe UUID
@VBoxManage internalcommands sethduuid %DOCKER_DEMO_VIRTMACH%\Controller\%DOCKER_DEMO_VDI_TEMPLATE_FILE%
::
:: Verwijderen VDI
...
@VBoxManage storageattach "ulx-s-2204-d-srvr" --storagectl "SATA" --port 0 --device 0 --type hdd --medium %DOCKER_DEMO_VIRTMACH%\Controller\%DOCKER_DEMO_VDI_TEMPLATE_FILE%
::
::
...
VBoxManage guestcontrol ulx-s-2204-d-srvr run --username=vagrant --password=vagrant "sudo sed 's@"1"@"0"@' -i /etc/apt/apt.conf.d/20auto-upgrades"
...
:: Foutmelding ! 
VBoxManage guestcontrol ulx-s-2204-d-srvr run --username=vagrant --password=vagrant "sudo curl -o /etc/netplan/00-installer-config.yaml https://raw.githubusercontent.com/jatutert/demos/main/Ansible/Virtualbox/Linux/Netplan/00-installer-config.yaml"
...
VBoxManage guestcontrol ulx-s-2204-d-srvr run --username=vagrant --password=vagrant "sudo hostnamectl set-hostname ulx-s-2204-d-srvr"
...
VBoxManage guestcontrol ulx-s-2204-d-srvr run --username=vagrant --password=vagrant "sudo sed 's@in.archive.ubuntu.com@nl.archive.ubuntu.com@' -i /etc/apt/sources.list"
