...
@vagrant box list -i generic/ubuntu2204
...
@VBoxManage -q --nologo startvm %DOCKER_DEMO_VM_HOSTNAME% --type=gui
...
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 -o StrictHostKeyChecking=no sudo sed 's@"1"@"0"@' -i /etc/apt/apt.conf.d/20auto-upgrades
:: 
...
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 sudo hostnamectl set-hostname ulx-s-2204-d-srvr
::
:: #### Aanpassen Ubuntu Repository 
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 sudo sed 's@mirrors.edge.kernel.org@nl.archive.ubuntu.com@' -i /etc/apt/sources.list
:: 
:: #### Updaten Ubuntu Repository
ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "sudo apt update -qq" >nul 2>&1
::
::
::
::
...
ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "sudo apt install docker.io -y"
@echo Installatie Docker met SNAP 
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "sudo snap install docker"
::
:: #### Groep docker aanmaken 
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "sudo groupadd docker"
:: 
...
@ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "sudo /home/vagrant/ubuntu-dckr-demo-config-V002.sh"
::
:: ####	Installatie Docker Compose Plugin 
:: 		https://gcore.com/learning/how-to-install-docker-compose-on-ubuntu/ 
::
ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "mkdir -p ~/.docker/cli-plugins/"
ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "curl -SL https://github.com/docker/compose/releases/download/v2.26.1/docker-compose-linux-x86_64 -o ~/.docker/cli-plugins/docker-compose"
ssh -i %USERPROFILE%\.ssh\id_rsa %DOCKER_DEMO_VAGRANT_LNX_USER%@127.0.0.1 -p 3000 "chmod +x ~/.docker/cli-plugins/docker-compose"
::
::  
:: 
